...
def fetch_measure_levels(measure_id, dt):
    ...
    for measure in data['items']:
        ...
        # NEW CODE checks if data keys exist
        if ('dateTime' in measure) and ('value' in measure):

            # Set d to entry date
            dat = measure['dateTime']
            if type(dat) is str:  # checks date is string
                # Convert date-time string to a datetime object
                dat = dateutil.parser.parse(dat)
            else:
                continue    # skip the entry if date invalid

            # Set lev to entry level
            lev = measure['value']
            if type(lev) is float:    # checks level is float
                pass    # pass to append
            elif (type(lev) is list) and len(lev) == 2:    # handle known special case
                lev = lev[1]  # set output to second entry in list for special case
            else:
                continue    # skip the entry if level invalid

            # Append date and level data
            dates.append(dat)
            levels.append(lev)
